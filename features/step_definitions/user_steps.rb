...
When /check the published date of the feed entry at position (\d*)$/ do |entry_position|
  published_text = AtomHelper.published_text(last_response.body,entry_position)
...
When /^I read the feed at "([^\"]*)"/ do |url|
  get url
end
...
Then /^The only entry's content should link to availability page from time period$/ do
  ...
  content.should match(/<a href="http:\/\/example.org\/availabilities\/#{only_availability.id}">#{expected_link_text}<\/a>/)
end
...
When /(?:I )?touch the availability at position (\d*) of the feed$/ do |entry_position|
  id = AtomHelper.entry_id(last_response.body,entry_position)
...
Then /the published date of the entry at position (\d*) has been updated$/ do |entry_position|
  published = Time.parse(AtomHelper.published_text(last_response.body,entry_position))
